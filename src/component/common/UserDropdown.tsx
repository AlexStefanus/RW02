...
const UserDropdown = () => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button onClick={() => setIsOpen(!isOpen)} className="user-dropdown-btn flex items-center space-x-2 text-gray-700 hover:text-[#00a753] px-3 py-2 text-sm font-medium smooth-transition rounded-md">
        <div className="user-avatar w-8 h-8 bg-[#00a753] rounded-full flex items-center justify-center">
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl border border-gray-200 z-50 animate-scale-in">
          <div className="py-1">
            ...
            <Link href="/dashboard" className="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-blue-50 hover:text-[#00a753] smooth-transition group" onClick={() => setIsOpen(false)}>
              <FiBarChart className="mr-3 group-hover:scale-110 transition-transform" size={16} />
              Dashboard
            </Link>

            <Link href="/dashboard/profile" className="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-blue-50 hover:text-[#00a753] smooth-transition group" onClick={() => setIsOpen(false)}>
