...
export const LoadingSpinner = ({ size = "md", message, className = "" }: LoadingSpinnerProps) => {
  ...
  return (
    <div className={`flex flex-col items-center justify-center py-8 ${className}`}>
      <div className={`animate-spin rounded-full border-b-2 border-[#00a753] ${sizeClasses[size]} mb-2`}></div>
...
export const ErrorState = ({ message, onRetry, className = "" }: ErrorStateProps) => {
  return (
    <div className={`flex flex-col items-center justify-center py-8 ${className}`}>
      ...
      {onRetry && (
        <button onClick={onRetry} className="bg-[#00a753] text-white px-4 py-2 rounded-lg hover:bg-[#008c45] transition-colors text-sm">
          Coba Lagi
        </button>
      )}
    </div>
