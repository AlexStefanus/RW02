$projectRoot = "C:\Users\Administrator\Documents\RW02"
...
function Test-ImportantComment {
    param (
        [string]$commentText
    )
...
foreach ($file in $files) {
    ...
    for ($lineIndex = 0; $lineIndex -lt $lines.Length; $lineIndex++) {
        ...
        if ($line -match '^\s*//') {
            if (Test-ImportantComment -commentText $line) {
                $newLines += $line
            } else {
                $fileModified = $true
            }
        ...
        if ($line -match '\{\s*/\*.*?\*/\s*\}') {
            ...
            foreach ($match in $jsxCommentMatches) {
                ...
                if (-not (Test-ImportantComment -commentText $jsxComment)) {
        ...
        if ($line -match '^\s*/\*') {
            ...
            $isImportantBlock = Test-ImportantComment -commentText $line
            
            while ($lineIndex + 1 -lt $lines.Length -and -not ($lines[$lineIndex] -match '\*/\s*$')) {
                $lineIndex++ 
                $commentBlockLines += $lines[$lineIndex]
                
                if (-not $isImportantBlock -and (Test-ImportantComment -commentText $lines[$lineIndex])) {
        ...
        if ($hasUrl -or (Test-ImportantComment -commentText $commentPart)) {
            $newLines += $line
        } else {
            $newLines += $codePart
            $fileModified = $true
